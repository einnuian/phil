# Ragas Section
Page: Evaluate and Improve a RAG App - Ragas
URL: https://docs.ragas.io/en/stable/howtos/applications/evaluate-and-improve-rag/
...
Learn more about working with datasets in [Core Concepts - Datasets](https://docs.ragas.io/en/stable/concepts/datasets/).

## Set up metrics for RAG evaluation

Now that we have our evaluation dataset ready, we need metrics to measure RAG performance. Start with simple, focused metrics that directly measure your core use case. More information on metrics can be found in [Core Concepts - Metrics](https://docs.ragas.io/en/stable/concepts/metrics/).
...
The experiment function runs your RAG system on each data sample and evaluates the response using our correctness metric. More information on experimentation can be found in [Core Concepts - Experimentation](https://docs.ragas.io/en/stable/concepts/experimentation/).
...
- Evaluates the response using the correctness metric  

- Returns detailed results including scores and reason
...
- **Analyze results in CSV**: View responses, metric scores and reasons

- **Deep-dive with traces**: Click the `mlflow_trace_url` in the results to jump directly to the detailed execution trace in MLflow UI for that evaluation
...
[](https://docs.ragas.io/en/stable/_static/imgs/howto_improve_rag_mlflow.png)
...
"What is the default repository type for create_repo?"
`model`
"The provided documents do not state the default repository type..."
**BM25 missed docs with create_repo details**

"What is the purpose of the BLIP-Diffusion model?"
"controllable text-to-image generation and editing"
"The provided documents do not mention BLIP‑Diffusion..."
**BM25 didn't retrieve BLIP-Diffusion docs**

"What is the name of the new Hugging Face library for hosting scikit-learn models?"
`Skops`
"The provided documents do not mention or name any new Hugging Face library..."
**BM25 missed Skops documentation**
...
**Create evaluation dataset**: Use real queries from your system or generate synthetic data with LLMs. 

**Define metrics**: Choose simple metrics aligned with your use case. Keep it focused.

**Run baseline evaluation**: Measure current performance and analyze error patterns to identify systematic failures.

**Implement targeted improvements**: Based on error analysis, improve retrieval (chunking, hybrid search), generation (prompts, models), or try agentic approaches.

**Compare and iterate**: Test improvements against baseline. Change one thing at a time until accuracy meets business requirements.
